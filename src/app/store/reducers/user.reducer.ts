...
export function userReducer(appUserState = initialLoginState, action): UserState {
  switch (action.type) {
    ...
    case UserActionTypes.ACTION_CHANGE_CARREER:
      return {
        ...appUserState,
        carreer:action.payload.carreer
        //The code below works same way the line above
        //...action.payload
      }
    ;
    case UserActionTypes.ACTION_CHANGE_INPUT_CARREER:
      return{
        ...appUserState,
        carreer:action.payload.carreer
        //The code below works same way the line above
...
export const selectUserJustCarreer = createSelector(selectUserState, (state) => state.carreer);
export const selectUserJustUser = createSelector(selectUserState, (state) => state.user);
